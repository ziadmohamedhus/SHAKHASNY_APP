...
import '../data/accounts_model.dart';

class AccountMedicines extends StatelessWidget {
  AccountMedicines({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => AppCubit()..GetAccoutantsMedicines(),
          ...
          return Scaffold(
            ...
            body: ConditionalBuilder(
              ...
              condition: state is! GetAppPatientLoadingState &&
                  cubit.accounts_model?.data != null,
              builder: (context) => ListView.builder(
                itemCount: cubit.accounts_model?.data?.length ?? 0,
                itemBuilder: (context, index) => Apponitments(context,
                    data: cubit.accounts_model!.data![index]),
              ),
            ),
          );
        },
  ...
  Widget Apponitments(context, {required AccountsData data}) {
    return Padding(
